...
<p align="center">
   <figure>
   <img src="/media/embedding_example.png" width="500" height="350"/>
   <figcaption>Figure: Visualization of embeddings of audio clips speaking a certain word</figcaption>
   </figure>
</p>
...
can be achieved by changing values of these elements without necessarily affecting the angles between $W_i$ and $x_i$ vectors. This means that optimizing for this loss doesn't explicitly promote separibility of embeddings belonging to different classes, neither does it push embeddings belonging to the same classes closer together.
...
- We normalize features $||x_i||$ by $l_2$ normalization and re-scale it by $s$.
...
We can improve this further by introducing an angular margin parameter $m$ to angle $\theta$ between $W_{y_i}$ and $x_i$. A margin encourages better class separation by penalizing the loss function if examples fall within a certain
...
<div align="center">
   ...
   <table>
      <tr>
         <td align="center">
            <img src="/media/vgg8_arcface_Training_Embeddings.gif" width="380" height="380" />
            <br />
            ArcFace 3D embeddings during training.
         </td>
         <td align="center">
            <img src="/media/vgg8_softmax_Training_Embeddings.gif" width="380" height="380" />
            <br />
...
As we can see that the arcface loss results in embedding clusters that are more cleanly separated with margin.
 
The provided code trains a VGG8 network on the popular MNIST dataset using the ArcFace loss and Softmax loss, and generate visualization of embeddings. For more information on how to create and visualize such embeddings, please checkout the repository.
