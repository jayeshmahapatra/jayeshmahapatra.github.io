...
The way this is done in practice is by calculating 3 vectors associated with each token:

1. **Query**: Encode what the token is looking for in other tokens
2. **Key**: Encode the kind of support this token provides to other token
3. **Value**: Encodes what this token actually contains

Using these 3 vectors, we get an output vector for each token.

Let's go through an example to encode the token $x_{1} = "Sam"$  in the sequence $x = Sam\space has\space a\space big\space house$  to see how the calculations are performed using these values.



<img src= "/media">

1. **Calculate Key, Query and Value Vectors**: Multiply each token embedding $x_i$ with learned weight vectors $w_q$, $w_q$ and $w_v$ to get key, query and value vectors $k_{i}$, $q_{i}$ and $v_i$ respectively.
2. **Calculate Scores**: Get scores $s_i$ for all the tokens in repect to "Sam" token by the equation $$s_{i} = q_1*k_{i}$$ where $i$ goes from $1$ to length of sequence $n$, and $q_i$, $k_i$ are the query vector and key vector for the token at $i$-th position.

3. **Divide and Softmax**: Then we get the final weights for each token (w.r.t "Sam") using the equation $$w^1_i = softmax(\frac{s_i}{d_k})$$ where i goes from $1$ to sequence length $n$. $d_k$ is the dimension of the key vector.

4. **Weighted Sum** : Finally we get the output vector for ""Sam" by taking a weighted average of all value vectors $v_i$ using calculated weights $w_i$. $$z_1 = \sum_{i=1}^{n} w^1_i \cdot v_i$$

In practice we do calculations for all these token at the same time using matrix multiplications. The equation becomes $$Z = softmax(\frac{Q\cdot K^T}{d_k})V$$ where
