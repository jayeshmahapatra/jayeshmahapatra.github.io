...
In the current golden age of ML applications, engineers are really spoiled for choice regarding their tools for creating a ML demo from python based frameworks like [Gradio](https://www.gradio.app/), [Streamlit](https://streamlit.io/) to traditional frontend frameworks like [React](https://react.dev/), [NextJS](https://nextjs.org/) etc.
...
A Virtual Private Server (VPS) is essentially a virtual machine leased from a hosting provider. This service allows users to customize various aspects such as storage, memory, CPU, GPU, and bandwidth based on their needs and budget.

Among the plethora of options available, [Amazon EC2](https://aws.amazon.com/ec2/) stands out as a widely recognized provider of dedicated virtual machines. However, it's worth noting that EC2 instances can be costly.

Personally, I opted for [Hetzner Cloud](https://www.hetzner.com/cloud/) due to its more affordable shared vCPU tiers, striking a favorable balance between performance and expenditure.
...
To deploy a new web application, I need followed the following steps:
