...
3. **Subword based tokenizers:** Create a vocabulary of subwords using some method. Split words into subwords that are then mapped to numberical representations.
...
3. **Calculate Output Encodings**: We get the final encoding for each token using the equation <br> $$ Z = softmax(\frac{S_{masked}}{\sqrt{d_k}}) \times V $$ <br> where $Z_{i}$ will contain encoding for $i$-th token.
...
In the multi headed case, we just maintain multiple weight matrices $ W^{Q}_{i} , W^{K}_{i}, W^{V}_{i} $ 
